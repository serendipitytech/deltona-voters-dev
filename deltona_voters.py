...
def summarize_voting_data(df, selected_elections, selected_voter_status, selected_commission_districts, selected_party):
    ...
    if 'Voting History' not in df.columns:
    ...
    all_party_summary.index = all_party_summary.index.map(
        lambda x, num_elections=len(selected_elections): f'{x[0]}, {x[1]}, {x[2]} of last {num_elections} elections'
    )

    # If a party is selected, filter the summary for the selected party
    if selected_party:
    ...
    row_totals_voting_history = selected_party_summary.sum(axis=1)
    column_totals_voting_history = selected_party_summary.sum(axis=0)
    ...
    return (
        summary_age, row_totals_age, column_totals_age, df[columns_for_detailed_age],
        selected_party_summary, row_totals_voting_history, column_totals_voting_history, df[columns_for_detailed_voting_history],
        selected_party_summary  # This includes the summary for all parties or the selected party
    )
