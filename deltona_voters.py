...
def summarize_voting_data_party(df, selected_elections, selected_voter_status, selected_commission_districts):
    race_mapping = {1: "Other", 2: "Other", 6: "Other", 9: "Other", 3: "African American", 4: "Hispanic", 5: "White"}
    df['Race'] = df['Race'].map(race_mapping)
    ...
    # Calculate summary for all parties
    summary_party_history = df.groupby(['Race', 'Sex', 'Voting History']).size().unstack(fill_value=0)
...
def page_2():
    ...
    # get the summaries and detailed records
    summary_age, row_totals_age, column_totals_age, detailed_age, summary_voting_history, row_totals_voting_history, column_totals_voting_history, detailed_voting_history, summary_party_history = summarize_voting_data_party(df, selected_elections, selected_voter_status, selected_commission_districts, selected_party)
    summary_age.index = summary_age.index.to_series().replace({'M': 'Male', 'F': 'Female', 'U': 'Unreported'}, regex=True)
    summary_voting_history.index = summary_voting_history.index.to_series().replace({'M': 'Male', 'F': 'Female', 'U': 'Unreported'}, regex=True)
